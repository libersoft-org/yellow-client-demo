<script>
    ...
    let items = [
        {
            id: 0, title: "Conversations", icon: "icon_nemp_v4.svg", hasDot: true, onclick: (e) => {
                e.preventDefault();
        ...
        {
            id: 1, title: "Newsfeed", icon: "icon_newsfeed.svg", hasDot: true, onclick: (e) => {
                e.preventDefault();
                $actualMVC = 'newsfeed';
                document.querySelector('#menu-toggle').click();
        ...
        {
            id: 3, title: "Video meeting", icon: "icon_video_meeting.svg", hasDot: true, onclick: () => {
                showVideoMeet = true;
            }
        },
...
<div class="social-icons" class:expanded={isExpanded}>
    {#if dndzone }
    <div class="social-icons-block" use:dndzone={{ items }} on:click={()=>{console.log('www');}} on:consider={handleDndConsider}
